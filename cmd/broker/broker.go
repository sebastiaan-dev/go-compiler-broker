...
import (
	...
	"github.com/sebastiaan-dev/go-compiler-broker/pkg/md"
...
func (broker *Broker) WorkerMsg(sender string, msg [][]byte) {
	//  At least, command
	if len(msg) == 0 {
		panic("len(msg) == 0")
	}

	command, msg := popBytes(msg)
	...
	case md.MDPW_READY:
		if worker_ready { //  Not first command in session
			...
			worker.Waiting()
			fmt.Printf("Added worker %q to service %s\n", sender, worker.service.name)
		}
	case md.MDPW_REPLY:
		if worker_ready {
			//  Remove & save client return envelope and insert the
			//  protocol header and service name, then rewrap envelope.
			client, msg := unwrapBytes(msg)
			broker.socket.SendMessage(client, "", md.MDPC_CLIENT, worker.service.name, msg)
			worker.Waiting()
		} else {
			worker.Delete(true)
		}
	case md.MDPW_HEARTBEAT:
		if worker_ready {
			worker.expiry = time.Now().Add(HEARTBEAT_EXPIRY)
		} else {
			worker.Delete(true)
		}
	case md.MDPW_DISCONNECT:
		worker.Delete(false)
	default:
...
func (broker *Broker) ClientMsg(sender string, msg [][]byte) {
	//  Service name + body
	if len(msg) < 2 {
		panic("len(msg) < 2")
	}

	service_frame, msg := popBytes(msg)
	service := broker.ServiceRequire(service_frame)

	//  Set reply return identity to client sender
	m := [][]byte{[]byte(sender), []byte("")}
	msg = append(m, msg...)
	...
	if len(service_frame) >= 4 && service_frame[:4] == "mmi." {
		...
		//  protocol header and service name, then rewrap envelope.
		client, msg := unwrapBytes(msg)
		broker.socket.SendMessage(client, "", md.MDPC_CLIENT, service_frame, msg)
	} else {
...
func (service *Service) Dispatch(msg [][]byte) {
	...
	for len(service.waiting) > 0 && len(service.requests) > 0 {
		...
		worker.Send(md.MDPW_REQUEST, "", msg)
	}
}
...
func (worker *Worker) Delete(disconnect bool) {
	if disconnect {
		worker.Send(md.MDPW_DISCONNECT, "", [][]byte{})
	}
	...
	for {
		...
		if time.Now().After(broker.heartbeat_at) {
			...
			for _, worker := range broker.waiting {
				worker.Send(md.MDPW_HEARTBEAT, "", [][]byte{})
			}
...
func unwrapBytes(msg [][]byte) (head []byte, tail [][]byte) {
	head = msg[0]
	if len(msg) > 1 && string(msg[1]) == "" {
		tail = msg[2:]
	} else {
		tail = msg[1:]
	}
...
func popBytes(ss [][]byte) (s string, ss2 [][]byte) {
	s = string(ss[0])
	ss2 = ss[1:]
	return
}
