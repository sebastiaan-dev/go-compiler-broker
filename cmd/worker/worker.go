...
func (mdwrk *Mdwrk) ConnectToBroker() (err error) {
	...
	err = mdwrk.SendToBroker(md.MDPW_READY, mdwrk.service, [][]byte{})
...
func (mdwrk *Mdwrk) Recv(reply [][]byte) (msg [][]byte, err error) {
	...
	if len(reply) > 0 {
		...
		m[1] = []byte("")
		err = mdwrk.SendToBroker(md.MDPW_REPLY, "", m)
	}
	...
	for {
		...
		if len(polled) > 0 {
			...
			if string(msg[1]) != md.MDPW_WORKER {
			...
			case md.MDPW_REQUEST:
				//  We should pop and save as many addresses as there are
				//  up to a null part, but for now, just save one...
				mdwrk.reply_to, msg = unwrapBytes(msg)
		...
		if time.Now().After(mdwrk.heartbeat_at) {
			mdwrk.SendToBroker(md.MDPW_HEARTBEAT, "", [][]byte{})
			mdwrk.heartbeat_at = time.Now().Add(mdwrk.heartbeat)
		}
	}
	return
...
func unwrapBytes(msg [][]byte) (head string, tail [][]byte) {
	head = string(msg[0])
